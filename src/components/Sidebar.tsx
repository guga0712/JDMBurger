...
export function Sidebar({ userName }: Props) {
  ...
  return (
    <>
      {/* ── Desktop sidebar ──────────────────────────────── */}
      <aside className="hidden md:flex w-56 shrink-0 flex-col bg-card border-r border-border h-screen sticky top-0">
          ...
          {NAV_ITEMS.map(({ href, label, icon: Icon, highlight, disable }) => {
            ...
            if (disable) {
              return (
                <span
                  key={href}
                  className="flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium text-muted-foreground/40 cursor-not-allowed select-none"
                >
      ...
      >
        ...
        <nav className="flex-1 flex flex-col px-8 gap-2">
          {NAV_ITEMS.map(({ href, label, icon: Icon, highlight, disable }, i) => {
            const isActive =
              href === '/pedidos'
                ? pathname === '/pedidos'
                : pathname === href || pathname.startsWith(href + '/')
            if (disable) {
              return (
                <span
                  ...
                  className={`flex items-center gap-4 px-4 py-4 rounded-xl text-lg font-semibold transition-all duration-300 text-muted-foreground/40 cursor-not-allowed select-none ${open ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'}`}
                >
